...
int minDistance(std::vector<std::vector<int>> &adjList, int source, int destination)
{
    ...
    std::cout << "distance: " << distance[destination] << "\n";
...
int main()
{
    ...
    std::vector<std::vector<int>> grid = processInput(source, destination);
    std::vector<std::vector<int>> adjList = makeAdjList(grid);
    ...
    for (int row = 0; row < grid.size(); row++)
    {
        for (int col = 0; col < grid[0].size(); col++)
        {
            if (grid[row][col] == 0)
            {
                std::cout << "source: " << (row * grid[0].size()) + col << "\n";
                minDis = std::min(minDis, minDistance(adjList, (row * grid[0].size()) + col, destination));
            }
        }
    }
