...
int main()
{
    ...
    const long NEEDED_SPACE = 30000000;
    const long UNUSED_SPACE = TOTAL_SPACE - directories["/"].size;
    const long DELETE_SIZE = NEEDED_SPACE - UNUSED_SPACE;
    ...
    {
        auto dir = itr.first;
        directorySizes.push_back(directories[dir].size);
    }

    std::sort(directorySizes.begin(), directorySizes.end(), std::greater<long>{});

    long deleteSize = 0;

    for (auto size : directorySizes)
    {
        if (size >= DELETE_SIZE)
        {
            deleteSize = size;
    ...
    std::cout << "FILTER VALUE: " << deleteSize << "\n";
}
